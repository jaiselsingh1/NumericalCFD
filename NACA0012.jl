...
function incompress_flow()
    ...
    ϵ = 1e-5
    residuals = Float64[]
    max_iter = 10000  # Add maximum iteration count
    iter = 0
    converged = false
        ...
        for i = 2:ix-1
            ...
            for j = 1:jx-1
                if j == 1
                    ...
                    else  # Symmetry plane update (j+1 = j-1)
                        Φ_next[i, j] = ((Φ[i+1, j] + Φ[i-1, j]) / Δx^2 + (2 * Φ[i, j+1] / Δy^2)) / (1 / h_sq)
                    end
                else
                    Φ_next[i, j] = ω * h_sq * ((1 / Δx^2) * (Φ[i+1, j] + Φ_next[i-1, j]) +
                                               (1 / Δy^2) * (Φ[i, j+1] + Φ_next[i, j-1]) +
                                               1 / h_sq * (1 - 1 / ω) * Φ[i, j])
                end
            end
        end
...
plt_slor_residual = plot(1:length(residuals), residuals,
    ...
    title="SOR Method (ω=1.8): NACA 0012 @ 0 deg AoA", yscale=:log10)
